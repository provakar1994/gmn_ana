void rootlogon(){
  ...
  FileStat_t buf;
  if( !gSystem->GetPathInfo("${PROJECT_BINARY_DIR}/libgmnana.so", buf) ){
    gSystem->Load("${PROJECT_BINARY_DIR}/libgmnana.so" ) ;
  }
  if( !gSystem->GetPathInfo("${PROJECT_BINARY_DIR}/libgmnana.dylib", buf) ){
    gSystem->Load("${PROJECT_BINARY_DIR}/libgmnana.dylib" ) ;
  }
}
