...
namespace util_pd {
  ...
  TH2D *TH2DHCALface_rc(std::string name) {
    // returns TH2D for HCAL face (row,col)
  ...
  //_____________________________________
  TH2D *TH2DHCALface_xy_data(std::string name) {
    ...
    TH2D *h = new TH2D(name.c_str(), ";yHCAL_{exp} (m);xHCAL_{exp} (m)",
		       expconst::hcalcol, y_min, y_max,
		       expconst::hcalrow, x_min, x_max);
    return h;
  }
  //_____________________________________
  TH2D *TH2DHCALface_xy_simu(std::string name) {
    // returns TH2D for HCAL face (x,y) [Simu]
  ...
  //_____________________________________
  TH2D *TH2DdxdyHCAL(std::string name) {
    // returns TH2D for dxdyHCAL
    TH2D *h = new TH2D(name.c_str(), "; yHCAL_{obs} - yHCAL_{exp} (m); xHCAL_{obs} - xHCAL_{exp} (m)",
  ...
  /* #################################################
     ...
     ################################################# */
  TH1D *TH1DhW(std::string name) {
    // returns W histogram
    TH1D *h = new TH1D(name.c_str(), "W Distribution (GeV)", 250,0,2);
    return h;
  }
  TH1D *TH1DhQ2(std::string name,       // Name of histogram
		int conf) {             // SBS config
    ...
    else std::cerr << "[Utilities::TH1DhQ2] Enter valid SBS config!!" << std::endl;
