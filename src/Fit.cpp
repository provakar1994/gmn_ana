...
namespace fit {
  ...
  /* NOTE: This fit method just fits the bg */
  {
    ...
    // 06/25/24 - Couldn't make the residual work! Still leaving it as is
    TH1F *hsANDbg = (TH1F*)hs->Clone(); hsANDbg->Add(hs,hbg);
    TH1F *hres = (TH1F*)hs->Clone(); hres->Add(ht,hsANDbg,1,-1); 
    ho = {ht_cp,hs,hbg,hres};
  ...
  /* TH Interpolation fit using 1 signal histo & 1 poly bg (1+Opoly+1 pars) */
  {
    ...
    bgf->SetNpx(500);
    bgf->SetParameters(&pars[1]);

    // now get a bg histo from bgf
    TH1F *hbg_sc = (TH1F*)hs_cp1->Clone(); util_pd::TF1toTH1F(bgf,hbg_sc);
  ...
  */
  {
    ...
    std::vector<double> setpars{1,xOff_range[1]}; for (int i=1;i<npars;i++) setpars.push_back(0);

    TH1F *ht_cp = (TH1F*)ht->Clone(); 
    TH1F *hs_cp1 = (TH1F*)hs->Clone(); 
 
    FitFn *ffn = new FitFn(hs_cp1,Opoly);
    TF1 *f1 = new TF1("f1",ffn,&FitFn::ffn_1hs_1pbg_xOffVary,fit_range[0],fit_range[1],npars);
    f1->SetNpx(2000);
    f1->SetParameters(&setpars[0]);
    f1->SetParName(0,"Norm");
    f1->SetParName(1,"xOff");
    set_poly_par_names(f1,2,Opoly);

    // setting limits for p and n peak offset varitations
    f1->SetParLimits(1,xOff_range[1],xOff_range[2]);
    ...
    TF1* bgf = new TF1("bgf",ffn2,&FitFn::ffn_poly,fit_range[0],fit_range[1],Opoly+1);
    bgf->SetNpx(500);
    bgf->SetParameters(&pars[2]);

    // now get a bg histo from bgf
    TH1F *hbg_sc = (TH1F*)hs_cp1->Clone(); util_pd::TF1toTH1F(bgf,hbg_sc);
    hbg_sc->SetMarkerColor(46); hbg_sc->SetLineColor(46); 
  ...
  /* TH Interpolation fit using 1 signal histo & 1 bg histo (2 pars) */
  {
    ...
    TH1F *hst = (TH1F*)hs_cp2->Clone(); hst->Add(hs_cp2,hbg_cp2);
    TH1F *hres = (TH1F*)ht_cp->Clone(); hres->Add(ht_cp,hst,1,-1); 
    ho = {ht_cp,hst,hres,hs_cp2,hbg_cp2};
  ...
  /* TH Interpolation fit using 2 signal histo (vary x offsets for both) & no background (2+2 pars) 
     This time x position of the signal histo is also free parameter
  */
  {
    const int npars = 4;
    ...
    FitFn *ffn = new FitFn(hs_cp1,hbg_cp1);
    TF1 *f1 = new TF1("f1",ffn,&FitFn::ffn_2hs_nbg_xOffVary,fit_range[0],fit_range[1],npars);
    f1->SetNpx(2000);
    f1->SetParameters(&setpars[0]);
    f1->SetParName(0,"Norm");
    f1->SetParName(1,"B");
    f1->SetParName(2,"xOff_s");
    f1->SetParName(3,"xOff_bg");
  ...
  */
  {
    ...
    f1->SetParName(0,"Norm");
    f1->SetParName(1,"R");
    f1->SetParName(2,"s1Off");
    f1->SetParName(3,"s2Off");
  ...
  */
  ...
  {
    const int npars = 4;
    std::vector<double> setpars{1,1,0,0};
    ...
    TH1F *hbg1_cp1 = (TH1F*)hbg1->Clone();
    TH1F *hbg2_cp1 = (TH1F*)hbg2->Clone();
    TH1F *hbg3_cp1 = (TH1F*)hbg3->Clone();    
 
    FitFn *ffn = new FitFn(hs1_cp1,hs2_cp1,hbg1_cp1,hbg2_cp1,hbg3_cp1);
    TF1 *f1 = new TF1("f1",ffn,&FitFn::ffn_2hs_3hbg,fit_range[0],fit_range[1],npars);
    f1->SetNpx(2000);
    f1->SetParameters(&setpars[0]);
    // f1->SetParName(0,"Norm");
    // f1->SetParName(1,"R");
    ...
    f1->SetParName(0,"#font[12]{N}");
    f1->SetParName(1,"#color[2]{#font[12]{R_{n/p}^{sf}}}");
    f1->SetParName(2,"#font[12]{B1}");
    f1->SetParName(3,"#font[12]{B2}");
    //f1->FixParameter(3,1);
    ...
    TH1F *hbg1_cp2 = (TH1F*)hbg1_cp1->Clone(); hbg1_cp2->Scale(pars[2]); 
    TH1F *hbg2_cp2 = (TH1F*)hbg2_cp1->Clone(); hbg2_cp2->Scale(pars[2]);
    TH1F *hbg_tot1 = (TH1F*)hbg1_cp2->Clone(); hbg_tot1->Add(hbg1_cp2,hbg2_cp2);     
    TH1F *hbg3_cp2 = (TH1F*)hbg3_cp1->Clone(); hbg3_cp2->Scale(pars[3]);     
    TH1F *hbg_sc = (TH1F*)hbg_tot1->Clone(); hbg_sc->Add(hbg_tot1,hbg3_cp2); 
    TH1F *hsANDbg = (TH1F*)hs1_cp2->Clone(); hsANDbg->Add(hst,hbg_sc);
    TH1F *hres = (TH1F*)hs2_cp1->Clone(); hres->Add(ht_cp,hsANDbg,1,-1); 
    ho = {ht_cp,hst,hbg_sc,hres,hs1_cp2,hs2_cp2,hbg_tot1,hbg3_cp2};
  ...
  TF1* fit_2hs_3hbg_THI_xOffVary (std::vector<double> const & fit_range,
				  ...
				  std::vector<double> const & xOff_range,  // ranges to vary hs1 & hs2 offsets
				  std::vector<TH1F*> &ho)    // Output: ht,hs,hbg,hres,N*hs1,N*R*hs2 (N=par[0],R=par[1])
  // TH Interpolation fit using 2 signal histos & 3 bg histos (6 pars)
  // x offsets are varied for both signal histos and the first and second hbg
  // B2 belongs to the third hbg 
  {
    const int npars = 6;
    std::vector<double> setpars{1,1,0,xOff_range[1],xOff_range[3],1};
