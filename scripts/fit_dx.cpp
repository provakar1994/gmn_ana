...
std::vector<std::string> replaceAndGenerateStrings(const std::string &mainStr, const std::vector<std::string> &cuts)
{
  ...
  for (const std::string &cut : cuts)
  {
    ...
    if (newStr.substr(0, 2) == "&&")
      newStr = newStr.substr(2);
    if (newStr.substr(newStr.size() - 2) == "&&")
      newStr = newStr.substr(0, newStr.size() - 2);
...
void customize_hsummary(TH1F *h, std::vector<std::string> const &lcuts)
{
  ...
  for (int i = 0; i < lcuts.size(); i++)
    h->GetXaxis()->SetBinLabel(i + 1, lcuts[i].c_str());
  h->LabelsOption("v", "X");
}
           ...
           bool is_elastic = 1) // 1=>Yes, 0=>QE
{
  ...
  // Choose proper weight branch for MC
  bool use_effi_corr = jmgr->GetValueFromSubKey<int>(key, "use_effi_corr");
  bool use_norm_weight = jmgr->GetValueFromSubKey<int>(key, "use_norm_weight");
  char const *weightn, *weightn_inel;  
  if (use_effi_corr)
  {
    if (use_norm_weight) {
      weightn = "weight_effic_norm";
      weightn_inel = "weight_norm";
    } else {
      weightn = "weight_effic";
      weightn_inel = "weight";
    }
  }
  else 
  ...
  std::vector<double> AR_w;
  jmgr->GetVectorFromSubKey<double>(key, "AR_width_x_y", AR_w);
  double sbs_kick = jmgr->GetValueFromSubKey<double>(key, "sbs_kick");
  std::vector<double> SM_w;
  jmgr->GetVectorFromSubKey<double>(key, "SM_width_xp_xn_y", SM_w);
  ...
  std::vector<double> pnXOff_range;
  jmgr->GetVectorFromSubKey<double>(key, "vary_pn_xOff_ranges", pnXOff_range);
  ...
  */
  ...
  TString outData = outFile;
  outData.ReplaceAll(".root", ".csv");
  ofstream outdata;
  ...
  vector<double> h_dx;
  jmgr->GetVectorFromSubKey<double>(key, "h_dx", h_dx);
  TH1F *h_dxHCAL_data = new TH1F("h_dxHCAL_data", "", int(h_dx[0]), h_dx[1], h_dx[2]);
  h_dxHCAL_data->Sumw2();
  ...
  TH1F *h_dxHCAL_simu_p = new TH1F("h_dxHCAL_simu_p", "", int(h_dx[0]), h_dx[1], h_dx[2]);
  h_dxHCAL_simu_p->Sumw2();
  TH1F *h_dxHCAL_simu_n = new TH1F("h_dxHCAL_simu_n", "", int(h_dx[0]), h_dx[1], h_dx[2]);
  h_dxHCAL_simu_n->Sumw2();
  ...
  TH1F *h_dxHCAL_bg_data1 = new TH1F("h_dxHCAL_bg_data1", "", int(h_dx[0]), h_dx[1], h_dx[2]);
  h_dxHCAL_bg_data1->Sumw2();
  TH1F *h_dxHCAL_bg_data2 = new TH1F("h_dxHCAL_bg_data2", "", int(h_dx[0]), h_dx[1], h_dx[2]);
  h_dxHCAL_bg_data2->Sumw2();
  TH1F *h_dxHCAL_bg_inel_p = new TH1F("h_dxHCAL_bg_inel_p", "", int(h_dx[0]), h_dx[1], h_dx[2]);
  h_dxHCAL_bg_inel_p->Sumw2();
  TH1F *h_dxHCAL_bg_inel_n = new TH1F("h_dxHCAL_bg_inel_n", "", int(h_dx[0]), h_dx[1], h_dx[2]);
  h_dxHCAL_bg_inel_n->Sumw2();
  TH1F *h_dxHCAL_bg_inel = new TH1F("h_dxHCAL_bg_inel", "", int(h_dx[0]), h_dx[1], h_dx[2]);
  h_dxHCAL_bg_inel->Sumw2();
  TH1F *h_dxHCAL_bg_random = new TH1F("h_dxHCAL_bg_random", "", int(h_dx[0]), h_dx[1], h_dx[2]);
  h_dxHCAL_bg_random->Sumw2();
  ...
  int nruns = -1;
  std::vector<CodaRun> cruns;
  util_pd::ReadRunList("../DB", nruns, conf, target, pass, sbsmag, 0, cruns);
  int minRnum = cruns[0].runnum - 1;
  int maxRnum = cruns[cruns.size() - 1].runnum + 2;
  std::vector<double> xbinsRnum;
  CalcRnumBinEdges(minRnum, maxRnum, 0, xbinsRnum);
  ...
  jmgr->GetVectorFromSubKey<double>(key, "cut_iter_min_width", cut_range);
  bool apply_to_data_only = jmgr->GetValueFromSubKey<int>(key, "apply_to_data_only");
  vector<double> h_cut_param;
  jmgr->GetVectorFromSubKey<double>(key, "h_cut_param", h_cut_param);
  ...
  customize_hcut(hcut);
  hcut->SetTitle(Form("%s {%s}", param_to_vary.c_str(), cuts_sig_data_modified.c_str()));
  ...
  customize_hcut_p(hcut_p);
  hcut_p->SetTitle(Form("%s {pCut&&%s}", param_to_vary.c_str(), cuts_sig_data_modified.c_str()));
  ...
  if (!is_elastic)
  {
    ...
    customize_hcut_n(hcut_n);
    hcut_n->SetTitle(Form("%s {nCut&&%s}", param_to_vary.c_str(), cuts_sig_data_modified.c_str()));
  }
  // // ** for simu
  ...
  if (!apply_to_data_only)
  {
    ...
    customize_hcut(hcut_simu);
    hcut_simu->SetTitle(Form("%s {%s}", param_to_vary.c_str(), cuts_sig_simu_modified.c_str()));
    ...
    customize_hcut_p(hcut_p_simu);
    hcut_p_simu->SetTitle(Form("%s {pCut&&%s}", param_to_vary.c_str(), cuts_sig_simu_modified.c_str()));
    ...
    if (!is_elastic)
    {
      ...
      customize_hcut_n(hcut_n_simu);
      hcut_n_simu->SetTitle(Form("%s {nCut&&%s}", param_to_vary.c_str(), cuts_sig_simu_modified.c_str()));
    }
  }
  ...
  std::vector<int> fvary_xp;
  jmgr->GetVectorFromSubKey<int>(key, "fidu_vary_xp", fvary_xp);
  std::vector<int> fvary_xn;
  jmgr->GetVectorFromSubKey<int>(key, "fidu_vary_xn", fvary_xn);
  std::vector<int> fvary_y;
  jmgr->GetVectorFromSubKey<int>(key, "fidu_vary_y", fvary_y);
  ...
  if (is_vary_cut)
  {
    ...
    {
      minval.push_back(low);
      maxval.push_back(high);
      std::string cut, cut_2;
      char low_buff[20];
      std::snprintf(low_buff, 20, "%.3f", low);
      std::string low_str(low_buff);
      char high_buff[20];
      std::snprintf(high_buff, 20, "%.3f", high);
      std::string high_str(high_buff);
      ...
      cuts.push_back(cut);
      cuts_2.push_back(cut_2);
      // defining cuts for MC
      if (apply_to_data_only)
        cut = "1";
      std::string cut_p = cut + "&&mc_fnucl==1";
      cuts_p.push_back(cut_p);
      std::string cut_n = cut + "&&mc_fnucl==0";
      cuts_n.push_back(cut_n);
      // update high and low
      if (cut_vary_style == 0)
      {
        low = high;
      ...
      {
        low = xrangeEqStat[i + 1];
        high = xrangeEqStat[i + 2];
      }
      else if (cut_vary_style == 3)
      {
        low += width;
  ...
  {
    cuts.push_back("1");
    cuts_p.push_back("mc_fnucl==1");
    cuts_n.push_back("mc_fnucl==0");
    cuts_2.push_back("N/A");
    minval.push_back(0);
    maxval.push_back(0);
    hcal_SMs.push_back(hcal_SM);
  }
  ...
  TString outGIF = outFile;
  outGIF.ReplaceAll(".root", ".gif");
  TString outPNG = outFile;
  outPNG.ReplaceAll(".root", "");

  // Fits
  vector<double> dx_fit_range;
  jmgr->GetVectorFromSubKey<double>(key, "dx_fit_range", dx_fit_range);
  vector<double> reject_points;
  jmgr->GetVectorFromSubKey<double>(key, "SB_reject_points", reject_points);
  ...
  std::vector<double> gfit_params;
  jmgr->GetVectorFromSubKey<double>(key, "Par_guess_of_gaus_bg", gfit_params);
  ...
  for (int i = 0; i < iter; i++)
  {
    ...
    { // don't use custom fidu cut
      h_dxHCAL_data = (TH1F *)data_rdf_filtered.Filter(cuts[i]).Histo1D({"h_dxHCAL_data", "", int(h_dx[0]), h_dx[1], h_dx[2]}, "dx")->Clone();
      if (use_norm_weight)
        h_dxHCAL_data_norm = (TH1F *)data_rdf_filtered.Filter(cuts[i]).Histo1D({"h_dxHCAL_data_norm", "", int(h_dx[0]), h_dx[1], h_dx[2]}, "dx", "weight_norm")->Clone();
      ...
      h_dxHCAL_bg_inel_p = (TH1F *)inel_rdf_filtered.Filter(cuts_p[i]).Histo1D({"h_dxHCAL_bg_inel_p", "", int(h_dx[0]), h_dx[1], h_dx[2]}, "dx_shifted_p", weightn_inel)->Clone();
      if (!is_elastic)
        h_dxHCAL_bg_inel_n = (TH1F *)inel_rdf_filtered.Filter(cuts_n[i]).Histo1D({"h_dxHCAL_bg_inel_n", "", int(h_dx[0]), h_dx[1], h_dx[2]}, "dx_shifted_n", weightn_inel)->Clone();
      h_dxHCAL_bg_inel = (TH1F *)h_dxHCAL_bg_inel_p->Clone();

      if (!is_elastic)
        h_dxHCAL_bg_inel->Add(h_dxHCAL_bg_inel_p, h_dxHCAL_bg_inel_n);
      // h_dxHCAL_bg_inel_p = (TH1F*)inel_rdf_filtered.Filter("mc_fnucl==1").Histo1D({"h_dxHCAL_bg_inel_p","",int(h_dx[0]),h_dx[1],h_dx[2]},"dx",weightn_inel)->Clone();
      // if (!is_elastic) h_dxHCAL_bg_inel_n = (TH1F*)inel_rdf_filtered.Filter("mc_fnucl==0").Histo1D({"h_dxHCAL_bg_inel_n","",int(h_dx[0]),h_dx[1],h_dx[2]},"dx",weightn_inel)->Clone();
      ...
      if (use_norm_weight)
        h2_dxHCAL_vs_rnum_norm = (TH2F *)data_rdf_filtered
                                     .Filter(cuts[i])
                                     .Histo2D({"h2_dxHCAL_vs_rnum_norm", "", nbinRnum, &(xbinsRnum)[0], int(h_dx[0]), h_dx[1], h_dx[2]}, "rnum", "dx", "weight_norm")
                                     ->Clone();
    }
    else
    { // use custom fidu cut
      h_dxHCAL_data = (TH1F *)data_rdf_filtered
                          ...
                          .Histo1D({"h_dxHCAL_data", "", int(h_dx[0]), h_dx[1], h_dx[2]}, "dx")
                          ->Clone();

      if (use_norm_weight)
        h_dxHCAL_data_norm = (TH1F *)data_rdf_filtered
                                 .Filter(cuts[i])
                                 .Filter(fiduCut, {"xHCAL", "yHCAL", "xHCAL_exp", "yHCAL_exp"})
                                 .Histo1D({"h_dxHCAL_data_norm", "", int(h_dx[0]), h_dx[1], h_dx[2]}, "dx", "weight_norm")
                                 ->Clone();
      h_dxHCAL_data_CT = (TH1F *)data_rdf_filtered.Filter(coinT_cut.c_str())
                             .Histo1D({"h_dxHCAL_data_CT", "", int(h_dx[0]), h_dx[1], h_dx[2]}, "dx")
                             ->Clone();
      h_dxHCAL_simu_p = (TH1F *)simu_rdf_filtered
                            .Filter(cuts_p[i])
                            .Filter(fiduCut, {"xHCAL", "yHCAL", "xHCAL_exp", "yHCAL_exp"})
                            .Histo1D({"h_dxHCAL_simu_p", "", int(h_dx[0]), h_dx[1], h_dx[2]}, "dx_shifted_p", weightn)
                            ->Clone();

      h_dxHCAL_simu_p_norm = (TH1F *)simu_rdf_filtered
                                 .Filter(cuts_p[i])
                                 .Filter(fiduCut, {"xHCAL", "yHCAL", "xHCAL_exp", "yHCAL_exp"})
                                 .Histo1D({"h_dxHCAL_simu_p_norm", "", int(h_dx[0]), h_dx[1], h_dx[2]}, "dx_shifted_p", "weight_norm")
                                 ->Clone();
      if (!is_elastic)
      {
        h_dxHCAL_simu_n = (TH1F *)simu_rdf_filtered
                              .Filter(cuts_n[i])
                              .Filter(fiduCut, {"xHCAL", "yHCAL", "xHCAL_exp", "yHCAL_exp"})
                              .Histo1D({"h_dxHCAL_simu_n", "", int(h_dx[0]), h_dx[1], h_dx[2]}, "dx_shifted_n", weightn)
                              ->Clone();
        h_dxHCAL_simu_n_norm = (TH1F *)simu_rdf_filtered
                                   .Filter(cuts_n[i])
                                   .Filter(fiduCut, {"xHCAL", "yHCAL", "xHCAL_exp", "yHCAL_exp"})
                                   .Histo1D({"h_dxHCAL_simu_n_norm", "", int(h_dx[0]), h_dx[1], h_dx[2]}, "dx_shifted_n", "weight_norm")
                                   ->Clone();
      }
      // kinematic histos "true"
      if (!apply_to_data_only && !is_elastic)
      ...
      h2_dxHCAL_vs_rnum = (TH2F *)data_rdf_filtered
                              .Filter(cuts[i])
                              .Filter(fiduCut, {"xHCAL", "yHCAL", "xHCAL_exp", "yHCAL_exp"})
                              .Histo2D({"h2_dxHCAL_vs_rnum", "", nbinRnum, &(xbinsRnum)[0], int(h_dx[0]), h_dx[1], h_dx[2]}, "rnum", "dx")
                              ->Clone();
      if (use_norm_weight)
        h2_dxHCAL_vs_rnum_norm = (TH2F *)data_rdf_filtered
                                     .Filter(cuts[i])
      ...
      // (data)
      if (apply_to_bg_data)
      { // applying custom fiduCut to data bg
        h_dxHCAL_bg_data1 = (TH1F *)bg_data_rdf_filtered1
                                .Filter(fiduCut, {"xHCAL", "yHCAL", "xHCAL_exp", "yHCAL_exp"})
                                .Histo1D({"h_dxHCAL_bg_data1", "", int(h_dx[0]), h_dx[1], h_dx[2]}, "dx")
                                ->Clone();
        h_dxHCAL_bg_data2 = (TH1F *)bg_data_rdf_filtered2
                                .Filter(fiduCut, {"xHCAL", "yHCAL", "xHCAL_exp", "yHCAL_exp"})
                                .Histo1D({"h_dxHCAL_bg_data2", "", int(h_dx[0]), h_dx[1], h_dx[2]}, "dx")
                                ->Clone();
      }
      else
      {
      ...
      // (simu)
      if (!apply_to_bg_simu)
      { // applying custom fiduCut to simu bg
        h_dxHCAL_bg_inel_p = (TH1F *)inel_rdf_filtered
                                 .Filter(cuts_p[i])
                                 .Filter(fiduCut, {"xHCAL", "yHCAL", "xHCAL_exp", "yHCAL_exp"})
                                 .Histo1D({"h_dxHCAL_bg_inel_p", "", int(h_dx[0]), h_dx[1], h_dx[2]}, "dx_shifted_p", weightn_inel)
                                 ->Clone();
        if (!is_elastic)
          h_dxHCAL_bg_inel_n = (TH1F *)inel_rdf_filtered
                                   .Filter(cuts_n[i])
                                   .Filter(fiduCut, {"xHCAL", "yHCAL", "xHCAL_exp", "yHCAL_exp"})
                                   .Histo1D({"h_dxHCAL_bg_inel_n", "", int(h_dx[0]), h_dx[1], h_dx[2]}, "dx_shifted_n", weightn_inel)
                                   ->Clone();
        // h_dxHCAL_bg_inel_p = (TH1F*)inel_rdf_filtered
        //   .Filter("mc_fnucl==1").Filter(fiduCut,{"xHCAL","yHCAL","xHCAL_exp","yHCAL_exp"})
        //   .Histo1D({"h_dxHCAL_bg_inel_p","",int(h_dx[0]),h_dx[1],h_dx[2]},"dx",weightn_inel)->Clone();
        // if (!is_elastic) h_dxHCAL_bg_inel_n = (TH1F*)inel_rdf_filtered
        // 		   .Filter("mc_fnucl==0").Filter(fiduCut,{"xHCAL","yHCAL","xHCAL_exp","yHCAL_exp"})
        // 		   .Histo1D({"h_dxHCAL_bg_inel_n","",int(h_dx[0]),h_dx[1],h_dx[2]},"dx",weightn_inel)->Clone();
      }
      else
      {
        h_dxHCAL_bg_inel_p = (TH1F *)inel_rdf_filtered.Filter(cuts_p[i]).Histo1D({"h_dxHCAL_bg_inel_p", "", int(h_dx[0]), h_dx[1], h_dx[2]}, "dx_shifted_p", weightn_inel)->Clone();
    ...
    if (is_vary_cut)
    {
      ...
      { // Anything other than fidu cut
        ...
        gPad->SetGridx();
        hcut_simu->Draw("HIST");
        hcut_p_simu->Draw("same HIST");
        if (!is_elastic)
          hcut_n_simu->Draw("same HIST");
        util_pd::PlotCutRegion(minval[i], maxval[i]);
      ...
      else
      {
        TH2F *h2_fiduCut_n = (TH2F *)data_rdf_filtered
                                 .Filter(fiduCut, {"xHCAL", "yHCAL", "xHCAL_exp", "yHCAL_exp"})
                                 .Histo2D({"h2_fiduCut_n", "", 200, -1, 1, 200, -3, 1.5}, "yHCAL_exp", "xHCAL_exp")
                                 ->Clone();
        TH2F *h2_fiduCut_p = (TH2F *)data_rdf_filtered
                                 .Filter(fiduCut, {"xHCAL", "yHCAL", "xHCAL_exp", "yHCAL_exp"})
                                 .Histo2D({"h2_fiduCut_p", "", 200, -1, 1, 200, -3, 1.5}, "yHCAL_exp", "xExp_shifted")
                                 ->Clone();
        ...
        TH2F *h2_fiduCut_p_MC = (TH2F *)simu_rdf_filtered
                                    ...
                                    ->Clone();
        //-- histo customization
        customize_h2fiduCut(h2_fiduCut_n, "n", "Data", 0);
        customize_h2fiduCut(h2_fiduCut_p, "p", "Data", sbs_kick);
        customize_h2fiduCut(h2_fiduCut_n_MC, "n", "MC", 0);
        customize_h2fiduCut(h2_fiduCut_p_MC, "p", "MC", sbs_kick);
        //---
    ...
    if (is_elastic)
    {
      ...
      TH1F *hf2 = (TH1F *)h_dxHCAL_data->Clone();
      util_pd::TF1toTH1F(f2, hf2);
      ho2[0]->Draw();
      c2->Update();
      ...
      p2[0]->cd();
      // plotting the histos
      h_dxHCAL_data->Draw("E");
      util_pd::customize_data(h_dxHCAL_data);
      hf2->Draw("same HIST");
      util_pd::customize_gfit(hf2, 1);
      ho2[1]->Draw("same HIST");
      util_pd::customize_nsig(ho2[1], 1);
      ho2[2]->Draw("same HIST");
      util_pd::customize_hbg(ho2[2], 1);
      ...
      p2[1]->cd();
      ho2[3]->Draw();
      util_pd::customize_residual(ho2[3]);
      ...
      // plotting the histos
      ho3[0]->Draw("E");
      util_pd::customize_data(ho3[0]);
      ho3[1]->Draw("same HIST");
      util_pd::customize_nsig(ho3[1], 1);
      ho3[2]->Draw("same HIST");
      util_pd::customize_hbg(ho3[2], 1);
      // redrawing the stat box
      st3->SetX1NDC(0.62);
      st3->SetX2NDC(0.9);
      st3->SetY2NDC(0.9);
    ...
    else
    {
      ...
      chi2.push_back(f0->GetChisquare());
      NDF.push_back(f0->GetNDF());
      R_vals.push_back(f0->GetParameter(1));
      Rerr_vals.push_back(f0->GetParError(1));
      B_vals.push_back(0);
      Berr_vals.push_back(0);
      pCnt.push_back(0);
      pCnt_err.push_back(0);
      nCnt.push_back(0);
      nCnt_err.push_back(0);
      bgCnt.push_back(0);
      bgCnt_err.push_back(0);
      // ho[0]->Draw("E"); util_pd::customize_data(ho[0]); customize_dx(ho[0]);
      ho[1]->Draw();
      customize_hs(ho[1]);
      ho[1]->SetStats(0);
      h_dxHCAL_data->Draw("E same");
      util_pd::customize_data(h_dxHCAL_data);
      // f0->Draw("same");
      ...
      if (is_vary_cut)
        AddCutToLegend(l0, cuts_2[i].c_str());
      l0->Draw();
      ...
      NDF.push_back(f1->GetNDF());
      R_vals.push_back(f1->GetParameter(1));
      Rerr_vals.push_back(f1->GetParError(1));
      B_vals.push_back(f1->GetParameter(2));
      Berr_vals.push_back(f1->GetParError(2));
      // converting fit fn to a hostogram
      TH1F *hf1 = (TH1F *)h_dxHCAL_data->Clone();
      util_pd::TF1toTH1F(f1, hf1);
      ho1[0]->Draw();
      c1->Update();
      // grabbing statbox of the fitted histo
      ...
      util_pd::customize_data(h_dxHCAL_data);
      hf1->Draw("same HIST");
      util_pd::customize_gfit(hf1, 1);
      ho1[4]->Draw("same HIST");
      util_pd::customize_psig(ho1[4], 1);
      ho1[5]->Draw("same HIST");
      util_pd::customize_nsig(ho1[5], 1);
      ho1[2]->Draw("same HIST");
      util_pd::customize_hbg(ho1[2], 1);
      // calculating yields
      std::vector<double> yo1;
      util_pd::GetYields(f1, ho1[4], ho1[5], ho1[2], yo1);
      pCnt.push_back(yo1[0]);
      pCnt_err.push_back(yo1[1]);
      nCnt.push_back(yo1[2]);
      nCnt_err.push_back(yo1[3]);
      bgCnt.push_back(yo1[4]);
      bgCnt_err.push_back(yo1[5]);
      // redrawing the stat box
      st1->SetX1NDC(0.65);
      st1->SetY1NDC(0.55);
      st1->SetX2NDC(0.95);
      ...
      if (is_vary_cut)
        AddCutToLegend(l1, cuts_2[i].c_str());
      l1->Draw();
      //
      // preparing the pad for residual
      //
      p1[1]->cd();
      ho1[3]->Draw();
      util_pd::customize_residual(ho1[3]);
      // drawing a horizontal line at y = 0
      ...
      // grabbing fit params for future use
      chi2.push_back(f2->GetChisquare());
      NDF.push_back(f2->GetNDF());
      R_vals.push_back(f2->GetParameter(1));
      Rerr_vals.push_back(f2->GetParError(1));
      B_vals.push_back(f2->GetParameter(2));
      Berr_vals.push_back(f2->GetParError(2));
      // converting fit fn to a hostogram
      TH1F *hf2 = (TH1F *)h_dxHCAL_data->Clone();
      util_pd::TF1toTH1F(f2, hf2);
      ho2[0]->Draw();
      ...
      hf2->Draw("same HIST");
      util_pd::customize_gfit(hf2, 1);
      ho2[4]->Draw("same HIST");
      util_pd::customize_psig(ho2[4], 1);
      ho2[5]->Draw("same HIST");
      util_pd::customize_nsig(ho2[5], 1);
      ho2[2]->Draw("same HIST");
      util_pd::customize_hbg(ho2[2], 1);
      // calculating yields
      std::vector<double> yo2;
      util_pd::GetYields(f2, ho2[4], ho2[5], ho2[2], yo2);
      pCnt.push_back(yo2[0]);
      pCnt_err.push_back(yo2[1]);
      nCnt.push_back(yo2[2]);
      nCnt_err.push_back(yo2[3]);
      bgCnt.push_back(yo2[4]);
      bgCnt_err.push_back(yo2[5]);
      // redrawing the stat box
      st2->SetX1NDC(0.65);
      st2->SetY1NDC(0.55);
      st2->SetX2NDC(0.95);
      ...
      if (is_vary_cut)
        AddCutToLegend(l2, cuts_2[i].c_str());
      l2->Draw();
      //
      // preparing the pad for residual
      //
      ...
      if (i > 0 && abs(f3->GetParameter(1)) < 10)
        setpars3 = fit::GetFitParams(f3); // Initial guess based on previous iteration
      if (is_vary_cut)
      {
      ...
      // grabbing fit params for future use
      chi2.push_back(f3->GetChisquare());
      NDF.push_back(f3->GetNDF());
      R_vals.push_back(f3->GetParameter(1));
      Rerr_vals.push_back(f3->GetParError(1));
      B_vals.push_back(f3->GetParameter(2));
      Berr_vals.push_back(f3->GetParError(2));
      // converting fit fn to a hostogram
      TH1F *hf3 = (TH1F *)h_dxHCAL_data->Clone();
      util_pd::TF1toTH1F(f3, hf3);
      ho3[0]->Draw();
      c3->Update();
      // grabbing statbox of the fitted histo
      TPaveStats *st3 = (TPaveStats *)ho3[0]->FindObject("stats");
      ...
      util_pd::customize_data(h_dxHCAL_data);
      hf3->Draw("same HIST");
      util_pd::customize_gfit(hf3, 1);
      ho3[4]->Draw("same HIST");
      util_pd::customize_psig(ho3[4], 1);
      ho3[5]->Draw("same HIST");
      util_pd::customize_nsig(ho3[5], 1);
      ho3[2]->Draw("same HIST");
      util_pd::customize_hbg(ho3[2], 1);
      // calculating yields
      std::vector<double> yo3;
      util_pd::GetYields(f3, ho3[4], ho3[5], ho3[2], yo3);
      pCnt.push_back(yo3[0]);
      pCnt_err.push_back(yo3[1]);
      nCnt.push_back(yo3[2]);
      nCnt_err.push_back(yo3[3]);
      bgCnt.push_back(yo3[4]);
      bgCnt_err.push_back(yo3[5]);
      // redrawing the stat box
      st3->SetX1NDC(0.67);
      st3->SetY1NDC(0.53);
      st3->SetX2NDC(0.95);
      ...
      l3->AddEntry(ho3[3], "Residual", "p");
      if (is_vary_cut)
        AddCutToLegend(l3, cuts_2[i].c_str());
      ...
      p3[1]->cd();
      ho3[3]->Draw();
      util_pd::customize_residual(ho3[3]);
      // drawing a horizontal line at y = 0
      util_pd::DrawZeroLine(p3[1], dx_fit_range[0], dx_fit_range[1]);
      // *******************
      ...
      if (i > 0 && abs(f4->GetParameter(1)) < 10)
        gfit_params = fit::GetFitParams(f4); // Initial guess based on previous iteration
      if (is_vary_cut)
      {
      ...
      // grabbing fit params for future use
      chi2.push_back(f4->GetChisquare());
      NDF.push_back(f4->GetNDF());
      R_vals.push_back(f4->GetParameter(1));
      Rerr_vals.push_back(f4->GetParError(1));
      B_vals.push_back(f4->GetParameter(2));
      Berr_vals.push_back(f4->GetParError(2));
      // converting fit fn to a hostogram
      TH1F *hf4 = (TH1F *)h_dxHCAL_data->Clone();
      util_pd::TF1toTH1F(f4, hf4);
      ho4[0]->Draw();
      c4->Update();
      // grabbing statbox of the fitted histo
      ...
      util_pd::customize_data(h_dxHCAL_data);
      hf4->Draw("same HIST");
      util_pd::customize_gfit(hf4, 1);
      ho4[4]->Draw("same HIST");
      util_pd::customize_psig(ho4[4], 1);
      ho4[5]->Draw("same HIST");
      util_pd::customize_nsig(ho4[5], 1);
      ho4[2]->Draw("same HIST");
      util_pd::customize_hbg(ho4[2], 1);
      // calculating yields
      std::vector<double> yo4;
      util_pd::GetYields(f4, ho4[4], ho4[5], ho4[2], yo4);
      pCnt.push_back(yo4[0]);
      pCnt_err.push_back(yo4[1]);
      nCnt.push_back(yo4[2]);
      nCnt_err.push_back(yo4[4]);
      bgCnt.push_back(yo4[4]);
      bgCnt_err.push_back(yo4[5]);
      // redrawing the stat box
      st4->SetX1NDC(0.67);
      st4->SetY1NDC(0.53);
      st4->SetX2NDC(0.95);
      ...
      if (is_vary_cut)
        AddCutToLegend(l4, cuts_2[i].c_str());
      ...
      p4[1]->cd();
      ho4[3]->Draw();
      util_pd::customize_residual(ho4[3]);
      // drawing a horizontal line at y = 0
      ...
      if (i > 0 && abs(f5->GetParameter(1)) < 10)
        setpars5 = fit::GetFitParams(f5); // Initial guess based on previous iteration
      if (is_vary_cut)
      {
      ...
      // grabbing fit params for future use
      chi2.push_back(f5->GetChisquare());
      NDF.push_back(f5->GetNDF());
      R_vals.push_back(f5->GetParameter(1));
      Rerr_vals.push_back(f5->GetParError(1));
      B_vals.push_back(f5->GetParameter(2));
      Berr_vals.push_back(f5->GetParError(2));
      // converting fit fn to a hostogram
      TH1F *hf5 = (TH1F *)h_dxHCAL_data->Clone();
      util_pd::TF1toTH1F(f5, hf5);
      ho5[0]->Draw();
      c5->Update();
      // grabbing statbox of the fitted histo
      ...
      util_pd::customize_data(h_dxHCAL_data);
      hf5->Draw("same HIST");
      util_pd::customize_gfit(hf5, 1);
      ho5[4]->Draw("same HIST");
      util_pd::customize_psig(ho5[4], 1);
      ho5[5]->Draw("same HIST");
      util_pd::customize_nsig(ho5[5], 1);
      ho5[2]->Draw("same HIST");
      util_pd::customize_hbg(ho5[2], 1);
      // calculating yields
      std::vector<double> yo5;
      util_pd::GetYields(f5, ho5[4], ho5[5], ho5[2], yo5);
      pCnt.push_back(yo5[0]);
      pCnt_err.push_back(yo5[1]);
      nCnt.push_back(yo5[2]);
      nCnt_err.push_back(yo5[3]);
      bgCnt.push_back(yo5[4]);
      bgCnt_err.push_back(yo5[5]);
      // redrawing the stat box
      st5->SetX1NDC(0.67);
      st5->SetY1NDC(0.48);
      st5->SetX2NDC(0.95);
      ...
      if (is_vary_cut)
        AddCutToLegend(l5, cuts_2[i].c_str());
      ...
      p5[1]->cd();
      ho5[3]->Draw();
      util_pd::customize_residual(ho5[3]);
      // drawing a horizontal line at y = 0
      ...
      // grabbing fit params for future use
      chi2.push_back(f6->GetChisquare());
      NDF.push_back(f6->GetNDF());
      R_vals.push_back(f6->GetParameter(1));
      Rerr_vals.push_back(f6->GetParError(1));
      B_vals.push_back(f6->GetParameter(2));
      Berr_vals.push_back(f6->GetParError(2));
      // converting fit fn to a hostogram
      TH1F *hf6 = (TH1F *)h_dxHCAL_data->Clone();
      util_pd::TF1toTH1F(f6, hf6);
      ho6[0]->Draw();
      c6->Update();
      // grabbing statbox of the fitted histo
      ...
      util_pd::customize_data(h_dxHCAL_data);
      hf6->Draw("same HIST");
      util_pd::customize_gfit(hf6, 1);
      ho6[4]->Draw("same HIST");
      util_pd::customize_psig(ho6[4], 1);
      ho6[5]->Draw("same HIST");
      util_pd::customize_nsig(ho6[5], 1);
      ho6[2]->Draw("same HIST");
      util_pd::customize_hbg(ho6[2], 1);
      // calculating yields
      std::vector<double> yo6;
      util_pd::GetYields(f6, ho6[4], ho6[5], ho6[2], yo6);
      pCnt.push_back(yo6[0]);
      pCnt_err.push_back(yo6[1]);
      nCnt.push_back(yo6[2]);
      nCnt_err.push_back(yo6[3]);
      bgCnt.push_back(yo6[4]);
      bgCnt_err.push_back(yo6[5]);
      // redrawing the stat box
      st6->SetX1NDC(0.6);
      st6->SetY1NDC(0.55);
      st6->SetX2NDC(0.95);
      ...
      if (is_vary_cut)
        AddCutToLegend(l6, cuts_2[i].c_str());
      l6->Draw();
      //
      // preparing the pad for residual
      //
      p6[1]->cd();
      ho6[3]->Draw();
      util_pd::customize_residual(ho6[3]);
      // drawing a horizontal line at y = 0
      ...
      // grabbing fit params for future use
      chi2.push_back(f7->GetChisquare());
      NDF.push_back(f7->GetNDF());
      R_vals.push_back(f7->GetParameter(1));
      Rerr_vals.push_back(f7->GetParError(1));
      B_vals.push_back(f7->GetParameter(2));
      Berr_vals.push_back(f7->GetParError(2));
      // converting fit fn to a hostogram
      TH1F *hf7 = (TH1F *)h_dxHCAL_data->Clone();
      util_pd::TF1toTH1F(f7, hf7);
      ho7[0]->Draw();
      c7->Update();
      // grabbing statbox of the fitted histo
      ...
      util_pd::customize_data(h_dxHCAL_data);
      hf7->Draw("same HIST");
      util_pd::customize_gfit(hf7, 1);
      ho7[4]->Draw("same HIST");
      util_pd::customize_psig(ho7[4], 1);
      ho7[5]->Draw("same HIST");
      util_pd::customize_nsig(ho7[5], 1);
      ho7[2]->Draw("same HIST");
      util_pd::customize_hbg(ho7[2], 1);
      // calculating yields
      std::vector<double> yo7;
      util_pd::GetYields(f7, ho7[4], ho7[5], ho7[2], yo7);
      pCnt.push_back(yo7[0]);
      pCnt_err.push_back(yo7[1]);
      nCnt.push_back(yo7[2]);
      nCnt_err.push_back(yo7[3]);
      bgCnt.push_back(yo7[4]);
      bgCnt_err.push_back(yo7[5]);
      // redrawing the stat box
      st7->SetX1NDC(0.65);
      st7->SetY1NDC(0.55);
      st7->SetX2NDC(0.95);
      ...
      if (is_vary_cut)
        AddCutToLegend(l7, cuts_2[i].c_str());
      l7->Draw();
      //
      // preparing the pad for residual
      //
      p7[1]->cd();
      ho7[3]->Draw();
      util_pd::customize_residual(ho7[3]);
      // drawing a horizontal line at y = 0
      ...
      if (!is_vary_cut && i == 0)
        c0->SaveAs(Form("%s[", outPlot.Data()));
      c0->SaveAs(Form("%s", outPlot.Data()));
      c0->SaveAs(Form("%s_0.png", outFileBase.Data()));
      c1->Update();
      c1->Write();
      c1->SaveAs(Form("%s", outPlot.Data()));
      c1->SaveAs(Form("%s_1.png", outFileBase.Data()));
      c2->Update();
      c2->Write();
      c2->SaveAs(Form("%s", outPlot.Data()));
      c2->SaveAs(Form("%s_2.png", outFileBase.Data()));
      c3->Update();
      c3->Write();
      c3->SaveAs(Form("%s", outPlot.Data()));
      c3->SaveAs(Form("%s_3.png", outFileBase.Data()));
      c4->Update();
      c4->Write();
      c4->SaveAs(Form("%s", outPlot.Data()));
      c4->SaveAs(Form("%s_4.png", outFileBase.Data()));
      c5->Update();
      c5->Write();
      c5->SaveAs(Form("%s", outPlot.Data()));
      c5->SaveAs(Form("%s_5.png", outFileBase.Data()));
      c6->Update();
      c6->Write();
      c6->SaveAs(Form("%s", outPlot.Data()));
      c6->SaveAs(Form("%s_6.png", outFileBase.Data()));
      c7->Update();
      c7->Write();
      c7->SaveAs(Form("%s", outPlot.Data()));
      c7->SaveAs(Form("%s_6.png", outFileBase.Data()));
      if (i == iter - 1)
        c7->SaveAs(Form("%s]", outPlot.Data()));
    } // QE
  } // for, cut vairation
