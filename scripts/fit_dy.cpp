/*
  This macro will fit dy distribtuion in various different ways.
  Plan:
  1. MC signal, no bg
  2. MC signal + poly bg
  -------
...
{
  h->GetXaxis()->SetTitle("y_{HCAL}^{obs} - y_{HCAL}^{exp} (m)");
}
...
int fit_dy (const char *configfilename, 
	    bool is_elastic = 1, // 1=>Yes, 0=>QE
	    std::string filebase="pdout/test_fit_dy") 
{
  ...
  double offset = jmgr->GetValueFromSubKey<double>(key,"dy_peak_offset_for_MC");
  std::string dy_shifted = "dy+" + std::to_string(offset);
  auto simu_rdf_filtered = simu_rdf_filtered_1.Define("dy_shifted",dy_shifted.c_str());
  auto bg_rdf_filtered = data_rdf.Filter(cuts_for_bg);

  // Creating important histograms
  vector<double> h_dy; jmgr->GetVectorFromSubKey<double>(key,"h_dy",h_dy);
  ...
  TH1F *h_dyHCAL_simu = (TH1F*)simu_rdf_filtered.Histo1D({"h_dyHCAL_simu","",int(h_dy[0]),h_dy[1],h_dy[2]},"dy_shifted","weight")->Clone();
  TH1F *h_dyHCAL_simu_p = (TH1F*)simu_rdf_filtered.Filter("mc_fnucl==1").Histo1D({"h_dyHCAL_simu_p","",int(h_dy[0]),h_dy[1],h_dy[2]},"dy_shifted","weight")->Clone();
  TH1F *h_dyHCAL_simu_n = (TH1F*)simu_rdf_filtered.Filter("mc_fnucl==0").Histo1D({"h_dyHCAL_simu_n","",int(h_dy[0]),h_dy[1],h_dy[2]},"dy_shifted","weight")->Clone();
  TH1F *h_dyHCAL_bg = (TH1F*)bg_rdf_filtered.Histo1D({"h_dyHCAL_bg","",int(h_dy[0]),h_dy[1],h_dy[2]},"dy")->Clone();

  // Fits
  vector<double> dy_fit_range; jmgr->GetVectorFromSubKey<double>(key,"dy_fit_range",dy_fit_range);

  // Canvas 0 : Fitting data/MC w/o any background
  TCanvas *c0 = util_pd::TC("c0",1,1);
  c0->cd(); gStyle->SetOptFit(1);
  vector<TH1F*> ho;
  TF1 *f0 = fit::fit_1hs_nbg_THI(dy_fit_range,
  ...
  vector<TH1F*> ho1;
  TF1 *f1 = fit::fit_1hs_1pbg_THI(dy_fit_range,
  				  h_dyHCAL_data,h_dyHCAL_simu,Opoly,
  				  ho1);
  ...
  TF1* bg1 = new TF1("bg1",ffn,&FitFn::ffn_poly,dy_fit_range[0],dy_fit_range[1],Opoly+1);
  bg1->SetNpx(500);
  ...
  // vector<TH1F*> ho2;
  // TF1 *f2 = fit::fit_1hs_1pbg_THI(dy_fit_range,
  // 				  h_dyHCAL_data,h_dyHCAL_data,Opoly,
  // 				  ho2);
