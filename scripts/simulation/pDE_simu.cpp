...
int pDE_simu(const char *configfilename) {
  ...
  ROOT::RDataFrame simu_rdf("Tout",Form("siout/%s_elas_ana_%s_sbs%d_sbs%dp_model%d.root",sfprefix.c_str(),gen.c_str(),conf,sbsmag,model));
  ...
  std::vector<double> hcal_AR = cut::hcal_active_area_simu(AR_w[0],AR_w[1]); 
  ...
  TH1F *h_xexp_earm = (TH1F*)earm_rdf.Histo1D({"h_xexp_earm","",int(h_xexp_lim[0]),h_xexp_lim[1],h_xexp_lim[2]},"xHCAL_exp","weight")->Clone();
  TH1F *h_xexp_eNharm = (TH1F*)eNharm_rdf.Histo1D({"h_xexp_eNharm","",int(h_xexp_lim[0]),h_xexp_lim[1],h_xexp_lim[2]},"xHCAL_exp","weight")->Clone();
  ...
  TH1F *h_yexp_earm = (TH1F*)earm_rdf.Histo1D({"h_yexp_earm","",int(h_yexp_lim[0]),h_yexp_lim[1],h_yexp_lim[2]},"yHCAL_exp","weight")->Clone();
  TH1F *h_yexp_eNharm = (TH1F*)eNharm_rdf.Histo1D({"h_yexp_eNharm","",int(h_yexp_lim[0]),h_yexp_lim[1],h_yexp_lim[2]},"yHCAL_exp","weight")->Clone();
  ...
  cdxdy->cd(1);
  TH2F *h_dxdy_earm = (TH2F*)earm_rdf.Filter("eHCAL>0").Histo2D({"h_dxdy_earm","",200,-1.5,1.5,200,-3,2},"dy","dx","weight")->Clone();
  util_pd::SetAxTitles(h_dxdy_earm,ytitledxdy,xtitledxdy);
  h_dxdy_earm->Draw("colz");
  cdxdy->cd(2);
  TH2F *h_dxdy_eNharm = (TH2F*)eNharm_rdf.Filter("eHCAL>0").Histo2D({"h_dxdy_eNharm","",200,-1.5,1.5,200,-3,2},"dy","dx","weight")->Clone();
  ...
  TH1F *h_w2_eNharm = (TH1F*)simu_rdf.Filter(global_cut.c_str()).Filter(harm_cut.c_str()).Histo1D({"h_w2_eNharm","",(int)h_w2_lim[0],h_w2_lim[1],h_w2_lim[2]},"W2","weight")->Clone();
