...
int npratio_simu(const char *configfilename) {
  ...
  ROOT::RDataFrame simu_rdf("Tout",Form("siout/%s_qelas_ana_%s_sbs%d_sbs%dp_model%d.root",sfprefix.c_str(),gen.c_str(),conf,sbsmag,model));
  ...
  auto p_rdf = simu_rdf.Filter(good_p_ev_cut.c_str());
  auto n_rdf = simu_rdf.Filter(good_n_ev_cut.c_str());
  ...
  TH1F *h_xexp_p = (TH1F*)p_rdf.Histo1D({"h_xexp_p","",int(h_xexp_lim[0]),h_xexp_lim[1],h_xexp_lim[2]},"xHCAL_exp","weight")->Clone();
  TH1F *h_xexp_n = (TH1F*)n_rdf.Histo1D({"h_xexp_n","",int(h_xexp_lim[0]),h_xexp_lim[1],h_xexp_lim[2]},"xHCAL_exp","weight")->Clone();
  ...
  TH1F *h_yexp_p = (TH1F*)p_rdf.Histo1D({"h_yexp_p","",int(h_yexp_lim[0]),h_yexp_lim[1],h_yexp_lim[2]},"yHCAL_exp","weight")->Clone();
  TH1F *h_yexp_n = (TH1F*)n_rdf.Histo1D({"h_yexp_n","",int(h_yexp_lim[0]),h_yexp_lim[1],h_yexp_lim[2]},"yHCAL_exp","weight")->Clone();
  ...
  cdxdy->cd(1);
  TH2F *h_dxdy_n = (TH2F*)n_rdf.Histo2D({"h_dxdy_n","",200,-1.5,1.5,200,-3,2},"dy","dx","weight")->Clone();
  h_dxdy_n->Draw("colz");
  cdxdy->cd(2);
  TH2F *h_dxdy_p = (TH2F*)p_rdf.Histo2D({"h_dxdy_p","",200,-1.5,1.5,200,-3,2},"dy","dx","weight")->Clone();
  h_dxdy_p->Draw("colz");
